<?
	...
	//ftp_id 검색시
	if($f_ftpid){
		$fsql = "select distinct pid from wo_ing02_host where ftpid like '%$f_ftpid%'";
		$fresult = mysql_query($fsql);
		$dtot = mysql_num_rows($fresult);
...
if($total_record != '0'){
	...
	while($row = mysql_fetch_array($result)){
		...
		for($h=0; $h<$hnum; $h++){
			...
			if($ftptype)	$iweb = $ftptype;
		}

		if($iweb)	 $iweb = "<img src='/admin/images/icon_iweb.gif' align='absmiddle'>";

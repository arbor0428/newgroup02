...
switch($type){
	...
	case 'edit' :
		...
		if($type=='write'){
			...
			$sql = "select * from wo_ing02e where sort >= $sort order by sort asc";
			...
			for($i=0; $i<$num; $i++){
				...
				mysql_query("update wo_ing02e set sort=$set_sort where uid=$set_uid");
			}
			...
			$sql = "insert into wo_ing02e  (sort,userid,name,company,host_id,host_pwd,upjong,person,staff,status,mobile,tel01,tel02,tel03,hp01,hp02,hp03,date01,edate01,edate02,edate03,price01,price02,price03,vat,domain,domain_com,domain_id,domain_pwd,site_id,site_pwd,domain_date,host_date,playing,ment ,reg_date,userfile01,realfile01) values ";
			$sql .= "('$sort','$userid','$name','$company','$host_id','$host_pwd','$upjong','$person','$staff','$status','$mobile','$tel01','$tel02','$tel03','$hp01','$hp02','$hp03','$date01','$edate01','$edate02','$edate03','$price01','$price02','$price03','$vat','$domain','$domain_com','$domain_id','$domain_pwd','$site_id','$site_pwd','$domain_date','$host_date','$playing','$ment ','$reg_date','$arr_new_file[1]','$real_name[1]')";
			$result = mysql_query($sql);
			$msg = '등록되었습니다';
			...
			if($old_sort < $sort){	 //높은순위로 변경시...
				...
				$sql = "select * from wo_ing02e where sort > $old_sort and sort <= $sort order by sort asc";
				$result = mysql_query($sql);
				$num = mysql_num_rows($result);
			...
			if($playing != '100%'){
				...
				//진행률이 100%일 아닌경우
				$sql = "select sort from wo_ing02e where uid=$uid";
				$result = mysql_query($sql);
				...
				$sql = "select * from wo_ing02e where uid='$uid'";
				$result = mysql_query($sql);
				$row = mysql_fetch_array($result);
				...
				$sql = "insert into wo_ing02  (sort,userid,name,company,host_id,host_pwd,upjong,person,staff,status,mobile,tel01,tel02,tel03,hp01,hp02,hp03,date01,edate01,edate02,edate03,price01,price02,price03,vat,domain,domain_com,domain_id,domain_pwd,site_id,site_pwd,domain_date,host_date,playing,ment ,reg_date,userfile01,realfile01) values ";
				$sql .= "('$max','$userid','$name','$company','$host_id','$host_pwd','$upjong','$person','$staff','$status','$mobile','$tel01','$tel02','$tel03','$hp01','$hp02','$hp03','$date01','$edate01','$edate02','$edate03','$price01','$price02','$price03','$vat','$domain','$domain_com','$domain_id','$domain_pwd','$site_id','$site_pwd','$domain_date','$host_date','$playing','$ment ','$reg_date','$userfile01','$realfile01')";
				...
				//완료테이블에서 삭제한다
				$sql = "delete from wo_ing02e where uid=$uid";
				$result = mysql_query($sql);

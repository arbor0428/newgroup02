...
$nquery = "and playing != '완료'";

if(!$aloneChk)	$aloneChk='미표시';
...
/************************************************* 등록 **********************************************************/
		...
		if($type=='write'){
			...
			$sql = "select * from wo_ing02 where sort >= $sort $nquery order by sort asc";
			...
			$sql = "insert into wo_ing02  (sort,userid,name,sales,company,host_id,host_pwd,upjong,person,staff,status,aloneChk,mobile,tel01,tel02,tel03,hp01,hp02,hp03,date01,edate01,edate02,edate03,price01,price02,price02_1,price03,price02_date,price02_1_date,vat,domain,domain_com,domain_id,domain_pwd,site_id,site_pwd,domain_date,host_date,virtualhost,playing,ment ,reg_date,userfile01,realfile01,email,fax01,fax02,fax03,site_name,re_name,cdate01,cdate02,cdate03,ctime,ftpDate) values ";
			$sql .= "('$sort','$userid','$name','$sales','$company','$host_id','$host_pwd','$upjong','$person','$staff','$status','$aloneChk','$mobile','$tel01','$tel02','$tel03','$hp01','$hp02','$hp03','$date01','$edate01','$edate02','$edate03','$price01','$price02','$price02_1','$price03','$price02_date','$price02_1_date','$vat','$domain','$domain_com','$domain_id','$domain_pwd','$site_id','$site_pwd','$domain_date','$host_date','$virtualhost','$playing','$ment ','$reg_date','$arr_new_file[1]','$real_name[1]','$email','$fax01','$fax02','$fax03','$site_name','$re_name','$cdate01','$cdate02','$cdate03','$ctime','$ftpDate')";
			...
			if($ment || $cost || $deposit || $deposit_date){
				$sql = "insert into wo_ing02_ment (pid,reg_date,ment,cost,cost_vat,deposit,deposit_date) values ('$pid','$reg_date','$ment','$cost','$cost_vat','$deposit','$deposit_date')";
				$result = mysql_query($sql);
			}
			...
			for($i=0; $i<$donum; $i++){
				...
				if($in_doname || $in_docom || $in_doid || $in_dopwd || $in_dodate){
					$sql = "insert into wo_ing02_domain (pid,doname,docom,dodate,doid,dopwd) values ('$pid','$in_doname','$in_docom','$in_dodate','$in_doid','$in_dopwd')";
					$result = mysql_query($sql);
				}
			}
			...
			$sql = "update wo_ing02 set domain_date='$dfast', domain='$dlist' where uid='$pid'";
			$result = mysql_query($sql);
			...
			for($i=0; $i<$honum; $i++){
				...
				if($in_ftpcapa || $in_ftptype || $in_ftpprice || $in_ftpid || $in_ftppwd || $in_hocom || $in_hoid || $in_hopwd || $in_hodate){
					$sql = "insert into wo_ing02_host (pid,ftpstate,ftptype,ftpid,ftppwd,hocom,hodate,hoid,hopwd,ftpcapa,ftpprice) values ('$pid','$in_ftpstate','$in_ftptype','$in_ftpid','$in_ftppwd','$in_hocom','$in_hodate','$in_hoid','$in_hopwd','$in_ftpcapa','$in_ftpprice')";
					$result = mysql_query($sql);
				}
			}
			...
			//에이젼시현황 테이블에 만료일이 가장빠른 날을 저장한다.
			$sql = "update wo_ing02 set host_date='$hfast' where uid='$pid'";
			$result = mysql_query($sql);
...
/************************************************* 수정 **********************************************************/
			...
			if($playing != '완료'){
				$sql = "select max(sort) from wo_ing02 where playing != '100%' order by sort asc";
				$result = mysql_query($sql);
...
/*
				...
				for($i=1; $i<=$num; $i++){
					...
					if($set_uid != $uid){
						mysql_query("update wo_ing02 set sort=$sno where uid=$set_uid");
					}
...
/*
				if($sort == '0'){
					$sql = "select max(sort) from wo_ing02 where playing!='100%'";
					$result = mysql_query($sql);
					$max = mysql_result($result,0,0);
					$sort = $max + 1;

				}else{
					$sql = "select * from wo_ing02 where sort >= $old_sort and sort <= $sort $nquery order by sort asc";
					$result = mysql_query($sql);
					$num = mysql_num_rows($result);
...
*/
			...
			if($ment || $cost || $deposit || $deposit_date){
				...
				if($mid)	$sql = "update wo_ing02_ment set ment='$ment',cost='$cost',cost_vat='$cost_vat',deposit='$deposit',deposit_date='$deposit_date' where uid='$mid'";
				else	$sql = "insert into wo_ing02_ment (pid,reg_date,ment,cost,cost_vat,deposit,deposit_date) values ('$uid','$reg_date','$ment','$cost','$cost_vat','$deposit','$deposit_date')";
			...
			$sql = "delete from wo_ing02_domain where pid='$uid'";
			$result = mysql_query($sql);
			...
			for($i=0; $i<$donum; $i++){
				...
				if($in_doname || $in_dodate){
					$sql = "insert into wo_ing02_domain (pid,doname,docom,dodate,doid,dopwd) values ('$uid','$in_doname','$in_docom','$in_dodate','$in_doid','$in_dopwd')";
					$result = mysql_query($sql);
				}
			}
			...
			//에이젼시현황 테이블에 만료일이 가장빠른 날을 저장한다.
			$sql = "update wo_ing02 set domain_date='$dfast', domain='$dlist' where uid='$uid'";
			$result = mysql_query($sql);
			...
			$sql = "delete from wo_ing02_host where pid='$uid'";
			$result = mysql_query($sql);
			...
			for($i=0; $i<$honum; $i++){
				...
				if($in_ftpcapa || $in_ftptype || $in_ftpprice || $in_ftpid || $in_ftppwd || $in_hocom || $in_hoid || $in_hopwd || $in_hodate){
					$sql = "insert into wo_ing02_host (pid,ftpstate,ftptype,ftpid,ftppwd,hocom,hodate,hoid,hopwd,ftpcapa,ftpprice) values ('$uid','$in_ftpstate','$in_ftptype','$in_ftpid','$in_ftppwd','$in_hocom','$in_hodate','$in_hoid','$in_hopwd','$in_ftpcapa','$in_ftpprice')";
					$result = mysql_query($sql);
				}
			...
			//에이젼시현황 테이블에 만료일이 가장빠른 날을 저장한다.
			$sql = "update wo_ing02 set host_date='$hfast' where uid='$uid'";
			$result = mysql_query($sql);
...
/************************************************* 삭제 **********************************************************/
	case 'del' :
		...
		//등록된 내용 삭제
		$sql = "delete from wo_ing02_ment where pid=$uid";
		$result = mysql_query($sql);
...
/************************************************* /삭제 **********************************************************/
	...
	case 'info_del' :
		...
		//등록된 내용 삭제
		$sql = "delete from wo_ing02_ment where uid=$mid";
		$result = mysql_query($sql);
	...
	case 'domain_del' :
		...
		//해당 도메인정보 삭제
		$sql = "delete from wo_ing02_domain where uid=$did";
		$result = mysql_query($sql);
	...
	case 'host_del' :
		...
		//해당 도메인정보 삭제
		$sql = "delete from wo_ing02_host where uid=$did";
		$result = mysql_query($sql);
...
/************************************************* 계약보류 **********************************************************/
	...
	case 'back' :
		...
		//위에서 등록한 영업현황의 uid값을 가져온다.
		$sql = "select uid from wo_ing01 where reg_date='$reg_date' order by uid desc limit 1";
		$result = mysql_query($sql);
		$pid = mysql_result($result,0,0);


		//에이젼시현황에 등록된 내용db를 가져온다.
		$sql = "select * from wo_ing02_ment where pid='$uid' order by uid asc";
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		...
		for($i=0; $i<$num; $i++){
			...
			$qu01 = "insert into wo_ing01_ment (pid,reg_date,ment,cost,cost_vat,deposit,deposit_date) values ('$pid','$info_reg_date','$info_ment','$info_cost','$info_cost_vat','$info_deposit','$info_deposit_date')";
			$qu02 = mysql_query($qu01);

		}


		//에이젼시현황에 등록된 내용db를 삭제한다.
		$sql = "delete from wo_ing02_ment where pid='$uid'";
		$result = mysql_query($sql);
		...
		//에이젼시현황에 등록된 도메인db를 가져온다.
		$sql = "select * from wo_ing02_domain where pid='$uid' order by uid asc";
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		...
		for($i=0; $i<$num; $i++){
			...
			$qu01 = "insert into wo_ing01_domain (pid,doname,docom,dodate,doid,dopwd) values ('$pid','$info_doname','$info_docom','$info_dodate','$info_doid','$info_dopwd')";
			$qu02 = mysql_query($qu01);
		}


		//에이젼시현황에 등록된 도메인db를 삭제한다.
		$sql = "delete from wo_ing02_domain where pid='$uid'";
		$result = mysql_query($sql);
		...
		//에이젼시현황에 등록된 호스팅db를 가져온다.
		$sql = "select * from wo_ing02_host where pid='$uid' order by uid asc";
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		...
		for($i=0; $i<$num; $i++){
			...
			$qu01 = "insert into wo_ing01_host (pid,ftpstate,ftptype,ftpid,ftppwd,hocom,hodate,hoid,hopwd,ftpcapa,ftpprice) values ('$pid','$info_ftpstate','$info_ftptype','$info_ftpid','$info_ftppwd','$info_hocom','$info_hodate','$info_hoid','$info_hopwd','$info_ftpcapa','$info_ftpprice')";
			$qu02 = mysql_query($qu01);
		}


		//에이젼시현황에 등록된 호스팅db를 삭제한다.
		$sql = "delete from wo_ing02_host where pid='$uid'";
		$result = mysql_query($sql);
...
if($class == 'goMsg'){
	Msg::goMsg($msg,$next_url.'?play_sort='.$play_sort.'&field='.$field);

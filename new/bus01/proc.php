...
//		if($ment)		$ment = addslashes($ment);
		...
		if($type=='write'){
			...
			$sql = "insert into wo_bus01  (mtype,userid,company,id,pwd,name,homepage,telephone,date01,account,ment,userfile01,realfile01,fax,email) values ";
			$sql .= "('$mtype','$userid','$company','$id','$pwd','$name','$homepage','$telephone','$date01','$account','$ment','$arr_new_file[1]','$real_name[1]','$fax','$email')";
	...
	case 'del' :
		...
		$sql = "select * from wo_bus01 where uid='$uid'";
		$result = mysql_query($sql);
		...
		$sql = "delete from wo_bus01 where uid=$uid";
		$result = mysql_query($sql);

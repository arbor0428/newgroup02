<?
	...
	$query = "select * from wo_bus01 $query_ment $sort_ment";
	...
	$query2 = "select * from wo_bus01 $query_ment $sort_ment limit $record_start, $record_count";
...
if($total_record != '0'){
	...
	while($row = mysql_fetch_array($result)){
		...
		if($homepage){
			$_hostInfo = parse_url($homepage);
			if($_hostInfo['scheme'] == '')		$_hostInfo['scheme'] = 'http';

			$linkUrl = $_hostInfo['scheme'].'://'.$_hostInfo['path'];
			if($_hostInfo['query'])	$linkUrl .= '?'.$_hostInfo['query'];

			$homepage = "<a href='$linkUrl' target='_blank'>".$homepage."</a>";
		}

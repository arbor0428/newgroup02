...
include $_SERVER["DOCUMENT_ROOT"]."/module/class/class.FileUpload.php";
...
switch ($type) {
	case 'write':
		...
		$row = sqlRowOne("select doc_num from wo_leave where doc_num like '$doc_num%' order by doc_num desc");
		if ($row == NULL) {
			$doc_num .= '001';
		} else {
		...
		$leave_uid = sqlRowOne("select uid from wo_leave where doc_num='$doc_num'");
	...
	case 'agree':
		...
		$result = sqlExe("update wo_leave set $target=$target_val where uid=$uid");
		...
		// status update
		$sql = "select userid, gubun, date1, sort1, date2, sort2, date_cnt, status, IFNULL(agree1_status, 2) a1, IFNULL(agree2_status, 2) a2, IFNULL(agree3_status, 2) a3, IFNULL(deal1_status, 2) d1, IFNULL(deal2_status, 2) d2, IFNULL(deal3_status, 2) d3 from wo_leave where uid=$uid";
		$row = sqlRow($sql);
		...
		$result2 = sqlExe("update wo_leave set status='$status' where uid=$uid");

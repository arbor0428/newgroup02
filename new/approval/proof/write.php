	...
	function setUserID() {
		...
		if (userid) {
			...
			pushDate(teamNum);
				...
				if(securi && securi2) {
					$('#regNumber').text(securi +' - '+ securi2.substr(0, 1)+'******'); // 주민등록번호 앞자리+뒷자리 텍스트로.
				} else {
					$('#regNumber').text(''); // 주민등록번호 앞자리+뒷자리 텍스트로.
				}

				if (addr01 && addr02) {
					$('#addr').text(addr01+' '+addr02); // 주소1+주소2 텍스트로.
				} else {
					$('#addr').text(''); // 주소1+주소2 텍스트로.
				}

				$('#team').text(team); // 소속
				$('#affil').text(affil);
				if (pdate01) { //퇴사자
					$('#date').text(idate01+'년 '+idate02+'월 '+idate03+'일 ~ '+pdate01+'년 '+pdate02+'월 '+pdate03+'일');
				} else { //재직자
					$('#date').text(idate01+'년 '+idate02+'월 '+idate03+'일 ~ <?=$today01?>년 <?=$today02?>월 <?=$today03?>일');
				}
				$('#drafter').text(name);
...
<input type='hidden' name='f_eDate' value='<?=$f_eDate?>'>
 ...
 <div class="container proof">
	<header>
		<h1>재&nbsp&nbsp&nbsp&nbsp직&nbsp&nbsp&nbsp&nbsp증&nbsp&nbsp&nbsp&nbsp명&nbsp&nbsp&nbsp&nbsp서</h1>
		<div class="bill_logo" style="background-color: none;">
